...
class Login(Frame):
    ...
    def __init__(self):
        ...
        self.host_label = Label(self.root, text="Host:").grid(row=0, column=0, sticky="W")
        self.host = Entry(self.root)
        self.host.grid(row=0, column=1)

        self.port_label = Label(self.root, text="Port:").grid(row=1, column=0, sticky="W")
        self.port = Entry(self.root)
        self.port.grid(row=1, column=1)

        self.channel_label = Label(self.root, text="Channel:").grid(row=2, sticky="W")
        self.channel = Entry(self.root)
        self.channel.grid(row=2, column=1)

        self.user_label = Label(self.root, text="User:").grid(row=3, sticky="W")
        self.user = Entry(self.root)
        self.user.grid(row=3, column=1)

        self.nick_label = Label(self.root, text="Nick:").grid(row=4, sticky="W")
        self.nick = Entry(self.root)
        self.nick.grid(row=4, column=1)

        self.password_label = Label(self.root, text="Password:").grid(row=5, sticky="W")
